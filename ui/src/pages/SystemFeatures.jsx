...
export default function SystemFeatures({ wizard, onChange }) {
  const systemFeatures = FEATURE_CATALOG.filter(f => f.level === 'system');
  const domains = [...new Set(systemFeatures.map(f => f.domain))];
  ...
  return (
    <div className="wizard-content">
      ...
      <div className="space-y-8">
        {domains.map(domain => {
          const featuresInDomain = systemFeatures.filter(f => f.domain === domain);
